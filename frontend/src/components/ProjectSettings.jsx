...
const ProjectSettings = ({ projectId, onClose, onSave }) => {
	...
	const {
		...
		error: projectError,
	} = useSWR(projectId ? `/api/projects/${projectId}` : null, api.fetcher);
	...
	const handleAddLabel = async () => {
		...
		if (labels.some((l) => l.name === trimmedLabel)) return;
	...
	const handleDeleteProject = async () => {
		if (
			window.confirm(
				"Are you sure you want to delete this project? This will also delete all associated images and annotations.",
			)
		) {
			try {
				...
				>
					<div className="settings-section">
						...
						<p className="section-desc">
							Configure the labels for this project. These labels will appear in
						...
						<div className="label-input">
							...
							<button
								...
								disabled={
									!newLabel.trim() ||
									labels.some((l) => l.name === newLabel.trim())
								}
							>
								Add
							</button>
					...
					{projectType !== "contrastive_learning" && (
						<div className="settings-section">
							...
							<div className="type-info">
								<p>
									<strong>{getProjectTypeName(projectType)}</strong>
								</p>
								<p className="small">
					...
					{projectType === "contrastive_learning" && (
						<div className="settings-section">
							...
							<p className="section-desc">
								This project type is used for selecting positive and negative
							...
							<div className="contrastive-info">
								...
								<p className="small">
									Negative examples will be highlighted in red.
								</p>
							</div>
						</div>
					)}
...
function getProjectTypeDescription(type) {
	const descriptions = {
		object_detection:
			"Draw bounding boxes around objects. Configure labels to name your annotations.",
		instance_segmentation:
			"Draw precise outlines for each object. Configure labels to name your segments.",
		contrastive_learning:
			"Select positive and negative examples for training contrastive models.",
	};
